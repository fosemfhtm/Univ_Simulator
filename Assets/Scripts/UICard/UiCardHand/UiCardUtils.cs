...
namespace Tools.UI.Card
{
    public class UiCardUtils : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if(UserInfo.MainCharacter.flag) { //UserInfo.MainCharacter.count == 3
                // UserInfo.MainCharacter.count = 0;
                // UserInfo.MainCharacter.enemyTurn();
                // UserInfo.MainCharacter.flag = false;
                while (CardHand.Cards.Count > 0)
                {
                    var randomCard = CardHand.Cards.RandomItem();
                    CardHand.PlayCard(randomCard);
                }
